...
data = pd.read_csv("Development/sample_data.csv")
data['module'] = data['file'].apply(lambda x : ast.literal_eval(x)['project'])
data['timestamp'] = pd.to_datetime(data.timestamp)
dfcountry = pd.read_csv('Development/countryMap.txt',sep='\t')
...
hist_data = pd.read_csv("Development/summ.csv")
...
def gen_tree_maps(n):
    ...
    width = 100.
    height = 100.
    sort_pack = sorted(top_packages.items(), key=lambda d: d[1], reverse=True)
    sort_pack_list = [tuple(i) for i in sort_pack]
    td = list(zip(*sort_pack_list))
    datal = list(td[1])
    textl = list(td[0])
    # values = list(top_packages.values())
    # names = list(top_packages.keys())
    circles = circ.circlify(datal)
    # print(circles)
    xyr = list(zip(*circles))
    xl = list(xyr[0])
    yl = list(xyr[1])
    rl = list(xyr[2])
    ...
    colorl = ['#00FFFF','#90EE90','#FFC0CB','#808080','#87CEFA']
    randidx = [random.randint(0,4) for x in range(0,len(datal))]
    ...
    figure = go.Figure(
                    data = [go.Scatter(
                        ...
                        name='hh',
                        mode='markers+text',
                        marker= {'color': [colorl[x] for x in randidx] , 'size': [x*250 for x in rl]}
                    )     ],
                    layout = go.Layout(
                        # autosize=True,
                        # width=600,
...
trend_data = pd.read_csv("Development/trend.csv")
